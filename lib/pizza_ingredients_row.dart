...
class _PizzaIngredientsRowState extends State<PizzaIngredientsRow> {
  ...
  void initState() {
    super.initState();
    _loadedPizza = fetchIngredientsByPizza(widget.pizza); 
  }
    ...
    return FutureBuilder<Pizza>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.done && snapshot.hasData) {
          ...
          return Text('Offre du moment');
        }
        return LinearProgressIndicator(); // Afficher un indicateur de chargement pendant le chargement des données
      },
    );
  }
